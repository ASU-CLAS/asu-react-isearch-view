...
import { IsearchCircleGrid } from "../containers/IsearchCircleGrid";
...
class IsearchDirectoryWrapperDrupal extends Component {
  ...
  processTitles(person) {
    ...
    if (person.titleIndex == -1) {
      if (person.working_title != undefined) {
        ...
        if (
          person.titles.raw != undefined &&
          person.titles.raw[0] != undefined
        ) {
          ...
          if (
            person.affiliations.raw != undefined &&
            person.affiliations.raw.includes("Student")
          ) {
            if (debug) {
          ...
          if (
            person.affiliations.raw != undefined &&
            person.affiliations.raw.includes("Courtesy Affiliate")
          ) {
            if (debug) {
    ...
    // if there is a sourceID index, use it to select the correct title from the titles array
      ...
      if (debug) {
        console.log("Set title via titleIndex = " + person.titleIndex);
      }
      ...
      .then((response) => {
        ...
        if (isearchConfig.type === "customList") {
            ...
            for (var i = 0; i < response.data.results.length; i++) {
              ...
              if (item === response.data.results[i].asurite_id.raw) {
                ...
                if (
                  ...
                  response.data.results[i].titleIndex = response.data.results[
                    i
                  ].deptids.raw.indexOf(
          ...
          orderedProfileResults = orderedProfileResults.map((item, index) => {
            ...
            item.titleIndex = item.deptids.raw.indexOf(
              isearchConfig.ids[0].toString()
            );
          ...
          if (typeof isearchConfig.selectedFilters !== "undefined") {
            ...
            const courtesyAffiliateEnabled = isearchConfig.selectedFilters.includes(
              "Courtesy Affiliate"
            );
            const emeritusEnabled = isearchConfig.selectedFilters.includes(
              "Emeritus"
            );
            ...
            function handleCourtesyAffiliates(profile) {
              if ("primary_simplified_empl_class" in profile) {
                ...
                const isCourtesyAffiliate = profile.affiliations.raw.includes(
                  "Courtesy Affiliate"
                );
  ...
  render() {
    ...
    } else if (config.displayType === "circle-grid") {
      return (
        <div>
          {config.showFilterAZ == true && (
            <IsearchAtoZFilter
          ...
          <IsearchCircleGrid profileList={results} listConfig={config} />
        </div>
      );
    }
  }
